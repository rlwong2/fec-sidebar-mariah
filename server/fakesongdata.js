...
const generateAPIinfo = function(username) {
  ...
  axios.get(`http://names.drycodes.com/1?nameOptions=funnyWords`)
    ...
    .then(function (response) {
      ...
      generateRandomLikeData(fakeArtistName, fakeSongName, fakeAlbumArt);
    })
  ...
  var generateRandomLikeData = function (fakeArtistName, fakeSongName, fakeAlbumArt) {

    var fakeUser = username
    var fakeSongName =
    var fakeArtistName = nameType;
    ...
    var fakeAlbumArt = //got to get links
...
const startGenerateRandomSongLikes = function(username) {
  ...
  while (i > 0) {
    generateAPIinfo(username, function() )
    i --;
  }
}
